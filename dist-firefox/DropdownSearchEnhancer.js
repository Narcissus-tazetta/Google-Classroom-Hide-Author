...
export class DropdownSearchEnhancer {
    constructor() {
        this.observer = null;
        this.enhancedDropdowns = new WeakSet();
    ...
    getDropdownItems(dropdown) {
        ...
        for (const selector of ITEM_SELECTORS) {
            const found = Array.from(dropdown.querySelectorAll(selector));
            if (found.length > 0) {
                items.push(...found);
            }
        }
        const allItems = items.length > 0 ? items : Array.from(dropdown.querySelectorAll('li, div[role="option"]'));
        return allItems.filter((item) => !item.hasAttribute("data-gc-search-item") && !item.querySelector(".gc-search-wrapper"));
    }
    enhanceDropdown(dropdown) {
        ...
        const input = searchLi.querySelector(".gc-search-input");
    ...
    setupSearchProtection(searchLi, dropdown) {
        ...
        const protectElement = () => {
            ...
            if (!wrapper) {
                ...
                const newInput = searchLi.querySelector(".gc-search-input");
                if (newInput) {
    ...
    setupDropdownObserver(input, dropdown) {
    ...
    async filterDropdown(dropdown, input) {
    ...
    async getNormalizedItems(dropdown) {
...
//# sourceMappingURL=DropdownSearchEnhancer.js.map